...
namespace Scenes.Battle.Combat.Logic
{
    ...
    public class SpearAttackLogic : CoinActionLogic
    {
        ...
        [Tooltip("창을 1개씩 '발사'할 때 순차 재생할 발사음 배열. 비워두면 발사음 없음 (기본 공격음은 BattleManager가 코인당 1회 재생).")]
            ...
            bool isFocus)
        {
            ...
            for (int i = 0; i < activeSpears.Count; i++)
            {
                ...
                if (spear != null)
                {
                    // 창 발사음: sequentialAttackSounds가 설정된 경우에만 창마다 순차 재생
                    // (기본 공격음 coin.attackSound는 BattleManager가 코인당 1회 이미 재생)
                    if (sequentialAttackSounds != null && sequentialAttackSounds.Length > 0)
                    {
                        attacker.PlayCharacterSfx(sequentialAttackSounds[i % sequentialAttackSounds.Length]);
                    }

                    // 순차 피격음(Hit) 결정 (콜백에 넘겨주기 위해 미리 계산)
                    ...
                    spear.Fire(target.transform, fireSpeed, (hitSpear) =>
                    {
                        if (target != null)
                        {
                            // 창 적중: 피격음(TakeDamage 내부 재생) + 대미지 + 피격 모션
                            target.TakeDamage(damagePerHit, currentHitSound);
